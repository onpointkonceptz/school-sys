...
def run():
    ...
    try:
        sf = StudentFee.objects.get(student__admission_number='KADWEL/25/006')
    except StudentFee.DoesNotExist:
        print("StudentFee for KADWEL/25/006 not found!")
        return
    ...
    for f in fees_class:
        print(f"  - {f.name}: Status='{f.student_status}', Term='{f.term}', Session='{f.session}'")
    ...
    if fees_status.count() == 0:
        print("MATCH FAILED!")
    else:
        print(f"MATCH SUCCESS! Total Amount: {sum(f.amount for f in fees_status)}")
